...
public final class CellValueParser {
    ...
    /**
     * Strips a trailing {@code /sec} from {@code val.toString()} and parses the result.
     * Returns {@code 0.0} when {@code val} is {@code null} or not parseable.
     *
     * @param val raw TPS cell value (e.g. {@code "10.50/sec"})
     * @return parsed TPS as a double
     */
    public static double parseTps(Object val) {
        if (val == null) return 0.0;
        return parseTps(val.toString());
    }
    ...
    /**
     * Strips a trailing {@code /sec} from {@code s} and parses the result.
     * Returns {@code 0.0} when {@code s} is {@code null}, blank, or not parseable.
     *
     * @param s TPS cell string (e.g. {@code "10.50/sec"})
     * @return parsed TPS as a double
     */
    public static double parseTps(String s) {
        if (s == null || s.isBlank()) return 0.0;
        try {
            return Double.parseDouble(s.replace("/sec", "").trim());
        } catch (NumberFormatException e) {
            return 0.0;
        }
    }
