...
import io.github.sagaraggarwal86.jmeter.listener.core.*;
...
public class AggregateReportPanel extends JPanel {
    ...
    private final JTextField tpsSlaField = new JTextField("", 5);
    ...
    private static boolean isValidTpsSla(String s) {
        if (s == null || s.isBlank()) return false;
        try {
    ...
    /**
     * @return TPS SLA field text
     */
    public String getTpsSla() {
        return tpsSlaField.getText().trim();
    }

    /**
     * @param v value; null treated as empty string
     */
    public void setTpsSla(String v) {
        tpsSlaField.setText(Objects.requireNonNullElse(v, ""));
    }
    ...
    private SlaConfig buildSlaConfig() {
        String tpsStr = tpsSlaField.getText().trim();
    ...
    private JPanel buildBottomPanel() {
        ...
        generateBtn.addActionListener(e -> {
            if (reportTypeCombo.getSelectedIndex() == 0) {
                refreshProviderCombo();
                aiReportLauncher.launch(generateBtn);
            } else {
                aiReportLauncher.launchSlaOnly(generateBtn);
            }
        });
    ...
    private void setupFieldListeners() {
        ...
        tpsSlaField.getDocument().addDocumentListener(slaRepaintListener);
    ...
    private void addPositiveIntFocusValidator(JTextField field, String message) {
        addPositiveNumberFocusValidator(field, message, false);
    }

    private void addPositiveNumberFocusValidator(JTextField field, String message,
