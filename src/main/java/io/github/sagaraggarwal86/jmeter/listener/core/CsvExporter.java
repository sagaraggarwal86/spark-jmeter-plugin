...
public final class CsvExporter {
    ...
    public void saveTableToCSV(File file) throws IOException {
        ...
        boolean hasTpsSla = sla.isTpsEnabled();
        ...
        try (BufferedWriter writer = new BufferedWriter(
            ...
            for (int row = 0; row < tableModel.getRowCount(); row++) {
                ...
                if (hasTpsSla) {
                    line.append(',').append(evaluateTpsSla(row, sla));
                }
                if (hasErrorSla) {
    ...
    /**
     * Evaluates the TPS SLA for one row.
     * TPS breaches when observed is <em>below</em> threshold (inverse of error/RT).
     *
     * @return {@code "PASS"}, {@code "FAIL"}, or {@code "-"}
     */
    private String evaluateTpsSla(int row, SlaConfig sla) {
